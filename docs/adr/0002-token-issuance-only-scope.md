...
The original design for the auth lambda called for three things: (1) validate the customer's CPF, (2) look up the customer's existence and status in the database, (3) generate and return a JWT. During scoping (see issue #4), the team agreed that implementing only **one** of the three — specifically token issuance — is sufficient for this phase.
...
The lambda performs the reduced, agreed-upon scope (**token issuance only** — no CPF format/checksum validation, no customer "status" check, and note there is no `status` column on `customer` in the schema at all today, so that check couldn't be implemented as literally specified regardless), **but keeps one minimal database read**: `SELECT c.user_id FROM customer WHERE c.cpf = $1`, followed by a roles lookup for that `user_id`. This is intentionally *not* the full validate-existence-and-status flow from the original design — it exists solely so the issued token's `user_id` is a real row, keeping `accept`/`reject` functional.
...
- Satisfies the agreed-upon scope (token issuance) without silently breaking existing, unrelated app functionality as a side effect.
...
- Still does not implement CPF format/checksum validation or a customer status check, so it's a deliberate partial implementation of the original three-part design, not a full one. Acceptable only because the scope was explicitly narrowed; would need revisiting if that changes.
...
- **A. Full three-part flow (CPF validation + status check + token).** The literal original design. Rejected per the agreed-upon reduced scope, and partly infeasible as specified regardless — there is no `status` column on `customer` in the actual schema, so "check existence and status" can only mean "check existence" today without a schema change nobody asked for.
